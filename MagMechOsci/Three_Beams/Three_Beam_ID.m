...
[freqs_up,x_POs_up,control_POs_up,t_POs_up]=get_POs_from_lvm('Forced_response_measurements/sweep_ampl_1p5_three_beams_wspring_up_0.lvm',strain_idx,f_idx,clock_beam_idx, N_periods,10^3);
freqs_up(end)=[];
x_POs_up(end)=[];
control_POs_up(end)=[];
t_POs_up(end)=[];
...
[freqs_down,x_POs_down,control_POs_down,t_POs_down]=get_POs_from_lvm('Forced_response_measurements/sweep_ampl_1p5_three_beams_wspring_down_0.lvm',strain_idx,f_idx,clock_beam_idx,N_periods,10^3);
...
clear x_POs_up x_POs_down control_POs_up control_POs_down t_POs_up t_POs_down
...
%%
list(:,1)=1:85;
list(:,2)=[2 1 3  5 4 6 13 8 14 16 15 17 7 9 11 10 12 18 20 19 21 25 23 26 22 24 27 49 34 50 52 51 53 29 35 37 36  38  54  56 55  57 61  59 62 58 60 63 28 30 32 31 33 39 41 40 42 46 44 47 43 45 48 64 66 65 67 71 69 72 68 70 73 80 77 81 75 78 82 74 76 79 83 84 85];
list(:,3)=[3 2 1 6 5 4 18 14 9 21 20 19 13 8 17 16 15 7 12 11 10 27 26 24 25 23 22 64 54 39 67 66 65 50 35 57 56 55 30 42 41 40 73 72 70 71 69 68 49 34 53 52 51 29 38 37 36 63 62 60 61 59 58 28 33 32 31 48 47 45 46 44 43 83 82 79 81 78 76 80 77 75 74 84 85];
%%
tols= [0.0348 0.016 0.009];  
...
[pars_exp(list(:,iter_dim),iter_dim), ~,err_rel(iter_dim)]=FourierID(Abig(:,list(:,iter_dim)),Bbig(iter_dim,:).',noise_lvl,tols(iter_dim),poly_str_tmp); %([1:83 250 251])([1:83 250 251])
end
...
pl(1,:)=plot(Om_vec./(2*pi),norm_c.*sum(squeeze(abs(z_hat(:,:,N_harm+1+iter_harm))),2),'x');
hold on
set(gca,'ColorOrderIndex',1)
plot(Om_sim./(2*pi),norm_c.*sum(squeeze(abs(x_hat_id(:,:,N_harm+1+iter_harm))),2),'Linewidth',2)

pl(2,:)=plot(Om_vec./(2*pi),norm_c.*squeeze(abs(z_hat(:,1,N_harm+1+iter_harm))),'x');
hold on
set(gca,'ColorOrderIndex',2)
plot(Om_sim./(2*pi),norm_c.*squeeze(abs(x_hat_id(:,1,N_harm+1+iter_harm))),'Linewidth',2)


pl(3,:)=plot(Om_vec./(2*pi),norm_c.*squeeze(abs(z_hat(:,2,N_harm+1+iter_harm))),'x');
hold on
set(gca,'ColorOrderIndex',3)
plot(Om_sim./(2*pi),norm_c.*squeeze(abs(x_hat_id(:,2,N_harm+1+iter_harm))),'Linewidth',2)


pl(4,:)=plot(Om_vec./(2*pi),norm_c.*squeeze(abs(z_hat(:,3,N_harm+1+iter_harm))),'x');
hold on
set(gca,'ColorOrderIndex',4)
plot(Om_sim./(2*pi),norm_c.*squeeze(abs(x_hat_id(:,3,N_harm+1+iter_harm))),'Linewidth',2)

legend(pl, 'q_1+q_2+q_2','q_1','q_2','q_3','location','NorthWest')
grid on
