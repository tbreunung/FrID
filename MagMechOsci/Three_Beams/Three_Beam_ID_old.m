...
[freqs_up,x_POs_up,control_POs_up,t_POs_up]=get_POs_from_lvm('sweep_ampl_1p5_three_beams_wspring_up_0.lvm',strain_idx,f_idx,clock_beam_idx, N_periods,10^3);
freqs_up(end)=[];
x_POs_up(end)=[];
control_POs_up(end)=[];
t_POs_up(end)=[];
...
x_hat_exp_up=NaN(length(freqs_up),dim,2*N_harm+1);
control_POs_up_hat=NaN(length(freqs_up),2*N_harm+1);

for iter_Om=1:length(freqs_up)
    control_POs_up_hat(iter_Om,:)=project_fmodes(t_POs_up{iter_Om},control_POs_up{iter_Om},N_harm,freqs_up(iter_Om)*2*pi);
    
    x_hat_exp_up(iter_Om,:,:)=project_fmodes( t_POs_up{iter_Om},x_POs_up{iter_Om},N_harm,freqs_up(iter_Om)*2*pi);
    
end

%project_fmodes(t_POs_norm{1},z_POs_un{1},N_harm,Om_vec_un(1))

[freqs_down,x_POs_down,control_POs_down,t_POs_down]=get_POs_from_lvm('sweep_ampl_1p5_three_beams_wspring_down_0.lvm',strain_idx,f_idx,clock_beam_idx,N_periods,10^3);
...
%   x_POs_down(freqs_down<36 | freqs_down>43.5 )=[];
%   control_POs_down(freqs_down<36 | freqs_down>43.5)=[];
%   t_POs_down(freqs_down<36 | freqs_down>43.5)=[];
%   freqs_down(freqs_down<36 | freqs_down>43.5)=[];
%x_POs_down=cellfun(@(c) [c(:,1) c(:,2) -c(:,3)],x_POs_down,'UniformOutput',false);
 
x_hat_exp_down=NaN(length(freqs_down),dim,2*N_harm+1);
control_POs_down_hat=NaN(length(freqs_down),2*N_harm+1);

for iter_Om=1:length(freqs_down)
    control_POs_down_hat(iter_Om,:)=project_fmodes(t_POs_down{iter_Om,:},control_POs_down{iter_Om},N_harm,freqs_down(iter_Om)*2*pi);

    x_hat_exp_down(iter_Om,:,:)=project_fmodes(t_POs_down{iter_Om},x_POs_down{iter_Om},N_harm,freqs_down(iter_Om)*2*pi);
end
...
figure
subplot(2,2,1)
tmp_ampl=cell2mat(cellfun(@max,cellfun(@abs,x_POs_up,'UniformOutput',false),'UniformOutput',false));

plot(freqs_up,tmp_ampl,'x')
 hold on
 tmp_ampl=cell2mat(cellfun(@max,cellfun(@abs,x_POs_down,'UniformOutput',false),'UniformOutput',false));
 plot(freqs_down,tmp_ampl,'x')
legend('3', '5' ,'6' )
 
title('max displacement')
%legend(['Up sig' num2str(strain_idx(1))],['Up sig' num2str(strain_idx(2))],['Down sig' num2str(strain_idx(1))],['Down sig' num2str(strain_idx(2))])

%ylim([0 0.1])
% 
for iter_harm=1:N_harm
    subplot(2,2,1+iter_harm)
    plot(freqs_up,abs(x_hat_exp_up(:,:,N_harm+1+iter_harm)),'x')
    hold on
    plot(freqs_down,abs(x_hat_exp_down(:,:,N_harm+1+iter_harm)),'x')
...
figure 
%plot(freqs,sweep.FED.phase(1,2:end)-sweep.FED.phase_control(1,2:end),'+')
%hold on
plot(Om_vec_un./(2*pi),angle(z_hat(:,:,3)),'s')
% figure
% subplot(1,2,1)
% plot(Om_vec_un,sum(tmp_ampl,2),'x')
% subplot(1,2,2)
% plot(Om_vec,2.*sum(abs(x_hat_duff(:,:,N_harm+2)),2),'x')
%%
...
%[pars_duff, res_duff]=FourierID(z_PO_duff,t_PO_duff,N_fmodes,Om_vec,poly_order,f_order,noise_lvl_sim,'stiff')
...
%[pars_exp, res_exp, err_rel]=FourierID(z_POs_un,t_POs_norm,N_fmodes,Om_vec_un,dim,poly_order,f_order,noise_lvl,tol);
...
%pars_sim=real(pars_exp(1:end-2*f_order*dim));
%pars_sim=reshape(pars_sim,length(pars_sim)/dim,dim).';
% pars_sim=NaN(sum(2:poly_order+2),1);
% 
% if strcmp(mode,'stiff')
%     stiff_ids=cumsum(1:poly_order);
%     pars_sim(stiff_ids)=pars_exp(1:poly_order);    
%     pars_sim(2)=pars_exp(poly_order+1);
% end
% if strcmp(mode,'stiff+damp')
%     stiff_ids=cumsum(1:poly_order);
%     pars_sim(stiff_ids)=pars_exp(1:poly_order);    
%     damp_ids=cumsum(2:poly_order+1);
%     pars_sim(damp_ids)=pars_exp(poly_order+1:2*poly_order);
% end
% if strcmp(mode,'full')
%     pars_sim=pars_exp;    
% end
...
ampl_id(isnan(ampl_id))=0;
 ...
 %pars_sim(1:3,4:6)=-pars_sim(1:3,4:6);
%real(pars_exp(poly_order+(2:poly_order))).'
IC=zeros(1,2*dim);
Om_sim=Om_vec;
...
%Abig3D=zeros(2*N_fmodes*dim,length(poly_str),N_orbits);
%Bbig3D=zeros(2*N_fmodes*dim,1,N_orbits);
%Abig=zeros(2*N_fmodes*dim*N_orbits,length(poly_str));
%Bbig=zeros(2*N_fmodes*dim*N_orbits,1);
parfor iter_POs=1:N_orbits
   ...
   % delte mean rows  
    %A0(N_fmodes+1:2*N_fmodes+1:end,:)=[];
    A0(N_fmodes+1,:)=[];
    % delete zeroth forcing coefficient
    A0(:,end-f_order)=[];
    %A0(:,end-f_order-(dim-1)*(2*f_order+1):2*f_order+1:end-f_order)=[];
    % delete zeroth acceleration coefficient
    B0(:,N_fmodes+1)=[];
    %B0(N_fmodes+1:2*N_fmodes+1:end)=[];
      
    %  Abig((iter_POs-1)*2*N_fmodes*dim+(1:2*dim*N_fmodes),:)=A0;
      Abig3D(:,:,iter_POs)=A0;
   % Abig_num=[Abig_num; A0(:,1:4) zeros(2*N_fmodes,2*(iter_POs-1))  A0(:,5:6) zeros(2*N_fmodes,2*(length(z_POs(:,1))-iter_POs))  ];
     % Bbig((iter_POs-1)*2*N_fmodes*dim+(1:2*dim*N_fmodes))=B0;
      Bbig3D(:,:,iter_POs)=B0;
...
%Abig=reshape(permute(Abig3D,[1 3 2]),2*N_fmodes*dim*N_orbits,length(poly_str));
%Bbig=reshape(Bbig3D,2*N_fmodes*dim*N_orbits,1);
...
for iter_pars=accept_idx+1:length(Abig(1,:))
    ...
    if  norm(Abig(:,iter_pars))<noise_lvl  || norm(P*Abig(:,iter_pars))/norm(Abig(:,iter_pars))>0.9
...
% for iter_poly=1:length(poly_str)
%     ploy_idx=poly_str{iter_poly}([11:4:end]);
%     
%     if length(ploy_idx)>2
%         if length(unique(ploy_idx))>1
%             del_idx=unique([del_idx iter_poly]);
%         end
%     end
% end
...
%        res_tmp=zeros(length(accept_idx)-1,1);
%        pars_tmp=zeros(length(accept_idx)-1);

        for iter_idx=1:length(accept_idx)
            A_reduce=Abig(:,[final_accept_idx  accept_idx(iter_idx)]);
            %A_reduce=Abig;
            %A_reduce(:,[accept_idx(iter_idx) del_idx])=[];
        ...
        %if res_min/norm(Bbig)<tol
            ...
            %disp(['Delete index: ' num2str(accept_idx(min_id))])
            disp(['Identified Polynomial:   ' poly_str{accept_idx(min_id)}])
            %disp(strjoin(['Polynomial: ' poly_str(accept_idx(min_id),1).'],'   '))
            disp(['Relative error: ' num2str(res_min/norm(Bbig))])
            %del_idx=[accept_idx(min_id) del_idx];
            final_accept_idx=[final_accept_idx accept_idx(min_id)];
            accept_idx(min_id)=[];
            pars=pars_tmp(:,min_id);
            res=res_min;
        else
            %disp(['No further sparsification without exceeding ' num2str(tol*100) '% relative error'])
