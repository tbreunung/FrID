...
[freqs_up,x_POs_up,control_POs_up,t_POs_up]=get_POs_from_lvm('Forced_response_measurements/sweep_two_beams_ampl_1p5_mag_0_up_spring.lvm',strain_idx,f_idx,clock_beam_idx, N_periods,10^3);
freqs_up(end)=[];
x_POs_up(end)=[];
control_POs_up(end)=[];
t_POs_up(end)=[];
...
[freqs_down,x_POs_down,control_POs_down,t_POs_down]=get_POs_from_lvm('Forced_response_measurements/sweep_two_beams_ampl_1p5_mag_0_down_spring.lvm',strain_idx,f_idx,clock_beam_idx,N_periods,10^3);
...
err_rel=zeros(dim,1);
for iter_dim=1:dim
    [pars_exp(:,iter_dim), ~,err_rel(iter_dim)]=FourierID(Abig,Bbig(iter_dim,:).',noise_lvl,tols(iter_dim),poly_str); %([1:83 250 251])([1:83 250 251])
end
...
%%
jump_idx=find(abs(diff(sum(z_ampl_id,2)))>0.2);
...
legend(pl,'q_1+q_2','q_1','q_2','location','NorthWest')
grid on
...
if ~isempty(del_idx)
    ...
    disp(strjoin(['Polynomials: ' poly_str(del_idx,1).'],'   '))
end
...
if res/norm(Bbig)<tol
    ...
    while chck==false
        ...
        if res_min/norm(Bbig)>tol
            ...
            disp(strjoin(['Identified Polynomial: ' poly_str(accept_idx(min_id),1).'],'   '))
